...
<html>
    ...
    title: {
        text: 'Grafico de humedad entre el <?php echo $fecha_ini;?> / <?php echo $fecha_fin;?>'
    },
    subtitle: {
    ...
    series: [{
        ...
        data: [
                 ...
                 <?php
           $sql= "SELECT NUMSERIE, TEMPERATURA,HUMEDAD, FECHA as 'Fecha', DATE(FECHA) as 'fech' from MEDIDA where FECHA >= '$fecha_ini' and FECHA <= '$fecha_fin' and NUMSERIE = '$serie';"; 
           $result = mysqli_query($conexion,$sql);
           while($consulta = mysqli_fetch_array($result))
           {
            ?>
               <?php echo $consulta['HUMEDAD']?>,
            <?php
           }        
         ?>
    ...
    <script type="text/javascript">
      function hacer_click(){    
        window.location="reporteH.php";
      }   
    </script>
      <?php
           $sql= "SELECT MAX(HUMEDAD), MIN(HUMEDAD) from MEDIDA where FECHA >= '$fecha_ini' and FECHA <= '$fecha_fin' and NUMSERIE = '$serie';"; 
           $result = mysqli_query($conexion,$sql);         
           while($consulta = mysqli_fetch_array($result))
           {         
               $con = $consulta['MAX(HUMEDAD)'];
               $con2 = $consulta['MIN(HUMEDAD)'];         
           }
           echo "Humedad máxmia: ".$con;
           echo "Humedad mínima: ".$con2;
      ?>
    </body>
